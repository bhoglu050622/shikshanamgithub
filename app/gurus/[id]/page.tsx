...
import instructorsDataRaw from '@/data/processed/instructors.json'

const instructorsData = instructorsDataRaw || [];
...
async function getInstructor(id: string): Promise<Instructor | null> {
  ...
  const instructor = instructorsData.find((inst: any) => inst.id === id)
...
export async function generateStaticParams() {
  if (!instructorsData || !Array.isArray(instructorsData) || instructorsData.length === 0) {
    return []
  }
...
export default async function InstructorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const instructorPosts = (allPosts && Array.isArray(allPosts) ? allPosts.filter((post: any) => post.author === instructor.name) : [])
