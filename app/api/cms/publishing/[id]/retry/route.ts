...
// POST /api/cms/publishing/[id]/retry - Retry failed publishing
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Retry publishing by setting status to published
    const updateData = {
    ...
    return NextResponse.json({
      success: true,
      message: `${contentType} republished successfully`,
      item: updatedItem
    })
  } catch (error) {
    ...
    console.error('Retry publishing error:', error)
