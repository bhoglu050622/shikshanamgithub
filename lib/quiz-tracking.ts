 ...
 */
export function saveQuizResult(quizId: string, result: any, userName?: string): void {
  try {
    ...
    localStorage.setItem(`${quizId}-result`, JSON.stringify(quizResult))

    // Update completed quizzes list
    const completedQuizzes = getCompletedQuizzes()
    if (!completedQuizzes.includes(quizId)) {
      completedQuizzes.push(quizId)
      localStorage.setItem('completed-quizzes', JSON.stringify({
        quizzes: completedQuizzes,
        lastUpdated: new Date().toISOString()
      }))
    }

    console.log(`Quiz ${quizId} result saved successfully`)
  } catch (error) {
...
export function getQuizResult(quizId: string): QuizResult | null {
  try {
    const stored = localStorage.getItem(`${quizId}-result`)
    if (stored) {
 ...
 */
export function areAllQuizzesCompleted(): boolean {
 ...
 */
export function getRemainingQuizzes(): string[] {
  const completed = getCompletedQuizzes()
  const totalQuizzes = ['guna-profiler', 'shiva-alignment']
