...
async function fetchAnalyticsMetrics(): Promise<DashboardMetrics> {
  try {
    // Use mock analytics data since CMS functionality is removed
    ...
    return {
      ...
      charts: {
        ...
        topPages: [
          { label: '/courses', value: Math.floor(totalViews * 0.4), change: Math.floor(Math.random() * 20) - 10 },
          { label: '/about', value: Math.floor(totalViews * 0.2), change: Math.floor(Math.random() * 20) - 10 },
          { label: '/blog', value: Math.floor(totalViews * 0.15), change: Math.floor(Math.random() * 20) - 10 },
          { label: '/schools', value: Math.floor(totalViews * 0.15), change: Math.floor(Math.random() * 20) - 10 },
          { label: '/contact', value: Math.floor(totalViews * 0.1), change: Math.floor(Math.random() * 20) - 10 },
        ],
        conversionFunnel: [
      ...
      realtime: {
        ...
        recentEvents: [
          { type: 'page_view', user: 'User123', action: 'Visited /courses', timestamp: Date.now() - 1000 },
          { type: 'course_view', user: 'User456', action: 'Viewed Sanskrit course', timestamp: Date.now() - 2000 },
          { type: 'enrollment', user: 'User789', action: 'Enrolled in Yoga course', timestamp: Date.now() - 3000 },
        ],
      },
      performance: {
        averageLoadTime: 250,
