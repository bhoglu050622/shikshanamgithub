...
interface SelfHelpSchoolsEditorProps {
  content: {
    title: string;
    subtitle: string;
    schools: Array<{
...
export default function SelfHelpSchoolsEditor({ content, onChange }: SelfHelpSchoolsEditorProps) {
  ...
  const updateField = (field: string, value: string) => {
    const updated = { ...schools, [field]: value };
    setSchools(updated);
    onChange(updated);
  };
  ...
  const addSchool = () => {
    ...
    const updated = {
      ...schools,
      schools: [...(schools.schools || []), newSchool]
    };
    setSchools(updated);
    onChange(updated);
  };
  ...
  const updateSchool = (index: number, field: string, value: string) => {
    const updatedSchools = [...(schools.schools || [])];
    updatedSchools[index] = { ...updatedSchools[index], [field]: value };
    const updated = { ...schools, schools: updatedSchools };
    setSchools(updated);
    onChange(updated);
  };

  const removeSchool = (index: number) => {
    const updatedSchools = (schools.schools || []).filter((_, i) => i !== index);
    const updated = { ...schools, schools: updatedSchools };
    setSchools(updated);
    onChange(updated);
  };
