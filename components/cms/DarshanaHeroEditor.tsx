...
interface DarshanaHeroEditorProps {
  content: {
    title: string;
    subtitle: string;
    description: string;
...
export default function DarshanaHeroEditor({ content, onUpdate }: DarshanaHeroEditorProps) {
  ...
  const addQuote = () => {
    ...
    setHero({
      ...hero,
      sanskritQuotes: [...(hero.sanskritQuotes || []), newQuote]
    });
  };
  ...
  const removeQuote = (id: string) => {
    setHero({
      ...hero,
      sanskritQuotes: (hero.sanskritQuotes || []).filter(quote => quote.id !== id)
    });
  };
  ...
  const updateQuote = (id: string, field: string, value: string) => {
    setHero({
      ...hero,
      sanskritQuotes: (hero.sanskritQuotes || []).map(quote =>
  ...
  return (
    <div className="space-y-6">
      <div className="grid gap-4">
        ...
        <div className="border-t pt-4">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold">Sanskrit Quotes</h3>
            <Button
              type="button"
              variant="outline"
              size="sm"
          ...
          <div className="space-y-4">
            {(hero.sanskritQuotes || []).map((quote, index) => (
              <div key={quote.id} className="border rounded-lg p-4">
                ...
                <div className="grid gap-4">
                  <div>
                    <Label htmlFor={`quote-text-${quote.id}`}>Sanskrit Text</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor={`quote-translation-${quote.id}`}>Translation</Label>
                    <Input
                      id={`quote-translation-${quote.id}`}
                  ...
                  <div>
                    <Label htmlFor={`quote-transliteration-${quote.id}`}>Transliteration</Label>
                    <Input
                      id={`quote-transliteration-${quote.id}`}
                      value={quote.transliteration}
                      onChange={(e) => updateQuote(quote.id, 'transliteration', e.target.value)}
                      placeholder="Enter transliteration"
                    />
                  </div>

                  <div>
                    <Label htmlFor={`quote-source-${quote.id}`}>Source</Label>
                    <Input
