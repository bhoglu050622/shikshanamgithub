...
const gurus = [
  ...
  },
  {
    ...
    color: 'from-turquoise-400 to-turquoise-600'
  },
  {
    ...
    color: 'from-wisdom-400 to-wisdom-600'
  },
  {
  ...
  },
  {
    ...
    color: 'from-turquoise-500 to-turquoise-700'
  },
  {
    name: 'Dr. Arjun Reddy',
    ...
    color: 'from-wisdom-500 to-wisdom-700'
  }
]

export default function MeetGurus() {
  return (
    <section id="meet-gurus" className="section-padding bg-white">
      <div className="container-custom">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          {gurus.map((guru, index) => (
            ...
            >
              <div className="bg-white rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-500 border border-gray-100 overflow-hidden relative">
                {/* Temple Bell Sound Animation */}
                <div className="absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                  <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-32 h-32 border-2 border-saffron-300 rounded-full animate-ping"></div>
                  <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-24 h-24 border-2 border-turquoise-300 rounded-full animate-ping animation-delay-1000"></div>
                </div>
                ...
                <div className="relative p-8 text-center">
                  ...
                  <h3 className="text-xl font-bold text-wisdom-900 mb-1 group-hover:text-saffron-600 transition-colors duration-300">{guru.name}</h3>
                  <p className="text-saffron-600 font-medium mb-2">{guru.title}</p>
                  <p className="text-wisdom-600 text-sm">{guru.specialization}</p>
                </div>
                ...
                <div className="px-8 pb-4">
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    <div className="text-center">
                      <div className="text-lg font-bold text-wisdom-900">{guru.rating}</div>
                      <div className="text-xs text-wisdom-600">Rating</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold text-wisdom-900">{guru.students}</div>
                      <div className="text-xs text-wisdom-600">Students</div>
                    </div>
                    <div className="text-center">
                      <div className="text-lg font-bold text-wisdom-900">{guru.courses}</div>
                      <div className="text-xs text-wisdom-600">Courses</div>
                    </div>
                  </div>
                </div>
                ...
                <div className="px-8 pb-6 opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0">
                  <div className="border-t border-gray-100 pt-4">
                    <p className="text-wisdom-700 text-sm mb-4 leading-relaxed">
                      {guru.description}
                    </p>
                    
                    <div className="mb-4">
                      <h4 className="font-semibold text-wisdom-900 mb-2 text-sm">Experience & Achievements:</h4>
                      <ul className="space-y-1">
                        {guru.achievements.map((achievement, idx) => (
                          <li key={idx} className="flex items-center space-x-2 text-xs text-wisdom-600">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-wisdom-500">{guru.experience} experience</span>
                      <motion.button
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                        className="flex items-center space-x-1 text-saffron-600 hover:text-saffron-700 text-sm font-medium"
                      >
        ...
        >
          <div className="bg-gradient-to-r from-saffron-50 to-turquoise-50 rounded-3xl p-8 max-w-2xl mx-auto">
            <h3 className="font-serif text-2xl font-bold text-wisdom-900 mb-4">
              Want to meet more Gurus?
            </h3>
            <p className="text-wisdom-700 mb-6">
              Discover our complete roster of authentic teachers and find the perfect guide for your journey.
            </p>
            <motion.button
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              className="bg-gradient-to-r from-saffron-600 to-turquoise-600 text-white px-8 py-3 rounded-2xl font-semibold hover:shadow-lg transition-all duration-300"
            >
              Meet All Gurus
            </motion.button>
          </div>
